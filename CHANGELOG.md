...
- Local model registry (`fiber-tracer model`).
- Experiment tracking store (`fiber-tracer experiment`).
- `fiber-tracer train` command with live JSON progress.
- Cross-platform `scripts/install.sh` and `scripts/verify.sh`.
- GitHub Actions install-and-verify workflow for Ubuntu, macOS, and Windows.
