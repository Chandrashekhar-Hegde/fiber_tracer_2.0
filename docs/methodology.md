...
Ground-truth validation uses deterministic synthetic phantoms generated by `fiber_tracer.validation.phantoms`. Fiber cylinders of known diameter, orientation, and length are rasterized onto a voxel grid at a chosen physical spacing, providing exact ground-truth labels and orientations for Dice and angular-error scoring. Available architectures include random, aligned, in-plane, orthogonal, woven, and twill orientation modes, plus short-fiber, recycled, and broken-fiber variants, with optional porosity.

### Voxelization and supersampling

By default each voxel is a hard in/out test at its center (`supersample = 1`), which produces aliased (staircase) fiber boundaries. Setting `supersample = s > 1` on `generate_fiber_phantom` evaluates each voxel at `s³` sub-voxel sample points and averages them, so boundary voxels take fractional values in `(0, 1)`. This anti-aliased rasterization reproduces the partial-volume effect of a real detector, giving phantom edges that resemble measured CT more closely than a binary rasterization. It improves edge realism rather than bulk volume-fraction accuracy, and the default leaves existing behavior unchanged.

XCT acquisition artifacts — beam hardening, ring artifacts, partial-volume blur, Poisson noise, and contrast jitter — can be layered on with the domain-randomization helpers in the same module to test robustness.
...
- **Phantom validation** uses synthetic fibers with controlled noise and optional artifacts; supersampled voxelization models partial-volume edges, but performance on real composites will still vary with contrast, noise, and fiber contacts.
